<template>
    <div>
        ...
        <form @submit.prevent="submit">
            <div class="form-group">
                <input id="email" type="email" class="form-control" :class="{ 'is-invalid': form.errors.has('email')}"
                       name="email" autofocus placeholder="E-Mail Address" v-model="form.email">
            ...
            <div class="form-group">
                <input id="password-confirm" type="password" class="form-control" :class="{ 'is-invalid': form.errors.has('password_confirmation')}"
                       name="password_confirmation" placeholder="Confirm Password" v-model="form.password_confirmation">
            </div>
        ...
        <div class="copy-text">
            <div class="d-block">
                <router-link class="mt-3" :to="{ name: 'login' }" tag="a">Log in</router-link>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            submit() {
                ...
                this.form.post('/api/auth/password/change').then(response => {
